...
function Data()
    ...
    α = 300.0 # grouping term
    β = 0.001 # Penalizes -ηm.
    γ = 8000.0 # Penalizes temperature outside of the dead-band
    ζ = 100.0 # Penalizes temperature rate of change.
    τ = 1500.0 # Penalizes control input ṁ
...
function get_model_dynamics(𝒟, k)
    A_const = zeros(3*N_homes*H, 3*N_homes*(H+1) + N_homes*H)
    B_const = zeros(3*N_homes*H)
    for i in 1:H
        A_const[(i-1)*3*N_homes+1:i*3*N_homes, (i-1)*3*N_homes+1:(i+1)*3*N_homes] = hcat(-(I+Δt*P.Aₘ), I)
        A_const[(i-1)*3*N_homes+1:i*3*N_homes, 3*N_homes*(H+1)+(i-1)*N_homes+1:3*N_homes*(H+1)+i*N_homes] = -Δt*P.Bₘ
        B_const[(i-1)*3*N_homes+1:i*3*N_homes, 1] = Δt*P.Eₘ*P.ψ(ti+Δt*(k-1)+Δt*(i-1)) + Δt*P.Fₘ*P.ω(ti+Δt*(k-1)+Δt*(i-1))
    end
...
function cent_MPC(𝒟, k)
    @suppress begin
        ...
        @variable(model, 0 <= m[1:N_homes, 1:H+1] <= 1)
        @variable(model, 0 <= T̃[1:N_homes, 1:H+1])
        @variable(model, ṁ[1:N_homes, 1:H])
        # Constraints
        @constraint(model, reshape(hcat(T[:,1], Ṫ[:,1], m[:, 1])',:,1) .== reshape(hcat(𝒟.T[:,k], 𝒟.Ṫ[:,k], 𝒟.m[:,k])',:,1))
        x = vcat(reshape(hcat(reshape(T,:,1), reshape(Ṫ,:,1), reshape(m,:,1))', :, 1), reshape(ṁ,:,1))
        ...
        # @constraint(model, dᵤ - dᵥ .== diff(sum(m, dims=1)', dims=1))

        @variable(model, 0 <= dᵤ[1:H+1])
        @variable(model, 0 <= dᵥ[1:H+1])
        if k == 1
            P_agg = sum(hcat(zeros(N_homes, 1),m), dims=1)'
        else
            P_agg = sum(hcat(𝒟.m[:, k-1],m), dims=1)'
        end
        π = diff(P_agg, dims=1)
...
function plant(𝒟, ṁₖ, k)
    ϵ = 0.0001
    if Int64(ceil((t_dr_i-ti)/Δt)) <= k <= Int64(floor((t_dr_f-ti)/Δt)) && DR_flag == 1 # Dictates when the Demand Response Occurs
        𝒟.m[:,k] .= 0 # If conditions are true, control signal sent to plant will be zero. This simulates a power outage.
        ṁₖ .= 0
    end
    ẋ = (I+Δt*P.Aₚ)*reshape(Array(hcat(𝒟.T[:,k], 𝒟.Ṫ[:,k], 𝒟.m[:,k])'),:,1) + Δt*P.Bₚ*ṁₖ + Δt*P.Eₚ*P.ψ(ti+Δt*(k-1)) + Δt*P.Fₚ*P.ω(ti+Δt*(k-1)) + ϵ * rand(3*N_homes)
    return ẋ, ṁₖ
end
...
function simulate()
    ...
    for k in 1:K
        ...
        time2 = @elapsed(
            if k >= 1 
                ṁ, m = cent_MPC(𝒟, k)
                ẋ, ṁ[:,1] = plant(𝒟, ṁ[:,1], k) 
                𝒟.T[:, k+1] = ẋ[1:3:end] # T
                𝒟.Ṫ[:, k+1] = ẋ[2:3:end] # Ṫ
                𝒟.m[:, k+1] = ẋ[3:3:end] # m
                𝒟.ṁ[:, k] = ṁ[:,1] # ṁ
                𝒟.mₚ[:, k:(k-1)+H+1] = m # Predicted control states.
            end
        )
        if k >= 2 
            RSL_wEF(𝒟, k)
