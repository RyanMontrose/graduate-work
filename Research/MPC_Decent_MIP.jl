...
function Data()
    ...
    ζ = 0.01 # Penalizes temperature rate of change.
    τ = 0.0 # Not used.
...
function set_model_dynamics(𝒟, k)
    # Emperical update conditions
    max_RoC = 25
    max_cond = 500
    for p in 1:N_homes
        if sum(abs.((𝒟.θᵣ[p][:,k] - 𝒟.θᵣ[p][:,k-1]) / Δt) .<= max_RoC) == 4 && cond(𝒟.𝒫[p]) <= max_cond
...
function get_model_dynamics(𝒟, k, p)
    A_const = zeros(2*H, 2*(H+1) + H)
    B_const = zeros(2*H, 1)
    for i in 1:H
        A_const[(i-1)*2+1:(i)*2, (i-1)*2+1:(i+1)*2] = hcat(-(I + Δt*P.h[p].Aₘ), I)
        A_const[(i-1)*2+1:(i)*2, (i)+(H+1)*2] = -Δt*P.h[p].Bₘ
        B_const[(i-1)*2+1:(i)*2, 1] = Δt*P.h[p].Eₘ*P.ψ(ti+Δt*(k-1)+Δt*(i-1)) + Δt*P.h[p].Fₘ*P.ω(ti+Δt*(k-1)+Δt*(i-1))
    end
...
function decent_MPC(𝒟, k, p)
    ...
    @suppress begin
        ...
        @variable(model, m[1:H], Bin)
        @variable(model, dᵤ[1:H+1] >= 0)
        @variable(model, dᵥ[1:H+1] >= 0)
        # @variable(model, ℓᵤ[1:H] >= 0)
        # @variable(model, ℓᵥ[1:H] >= 0)
        # Constraints
        @constraint(model, vcat(T[1], Ṫ[1]) .== vcat(𝒟.T[p, k], 𝒟.Ṫ[p, k])) # Plants state response becomes initial condition for mpc controller.
        @constraint(model, T̃ .>= T .- P.h[p].DB_range[2])
        @constraint(model, T̃ .>= P.h[p].DB_range[1] .- T)
        A, B = get_model_dynamics(𝒟, k, p)
        @constraint(model, A * vcat(reshape(hcat(T, Ṫ)', :, 1),  m) .== B)
        for i in 1:H
            if 𝒟.lkout[p, k+(i-1)] == 1
                @constraint(model, m[i] == 0)
            end
        end
        # @constraint(model, mᵤ .- mᵥ .== diff(m))
        # for j in 1:H-𝒟.lkout_steps-1
        #     @constraint(model, sum(mᵤ[j:j+𝒟.lkout_steps-1] .+ mᵥ[j:j+𝒟.lkout_steps-1]) <= 1.0 )
        # end

        @constraint(model, dᵤ .- dᵥ .== Ṫ)

        # for i in 1:H
        #     @constraint(model, ℓᵤ[i] - ℓᵥ[i] == m[i] - sum(𝒟.mₚ[j,k+(i-1)] for j in findall(𝒟.𝒜[p,:] .== 1)) / length(findall(𝒟.𝒜[p,:] .== 1)))
        # end
...
function plant(𝒟, mₖ, k, p)
    ...
    ẋ = (I + Δt*P.h[p].Aₚ)*vcat(𝒟.T[p, k], 𝒟.Ṫ[p, k]) + Δt*P.h[p].Bₚ*mₖ + Δt*P.h[p].Eₚ*P.ψ(ti+Δt*(k-1)) + Δt*P.h[p].Fₚ*P.ω(ti+Δt*(k-1)) + ϵ * rand(2)
    return ẋ, mₖ
end
...
# 𝒟 = simulate()
# @save "DD_data_n1000.bson" 𝒟
# @save "DD_P_n1000.bson" P
# @save "DD_data.bson" 𝒟
# @save "DD_P.bson" P

# @load "DD_data.bson" 𝒟
# @load "DD_P.bson" P
@load "DD_data_n1000.bson" 𝒟
@load "DD_P_n1000.bson" P
